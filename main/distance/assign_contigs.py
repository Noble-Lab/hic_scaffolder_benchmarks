...
def run_mummer(reference, assembly, threads):
	...
	alignment = read('coordinates.txt')
...
def remove_alignments(group, min_length = 1000, min_coverage = 0.2):
...
def get_assignments(df, reference_lengths, assembly_lengths):
	...
	for scaffold_name in reference_lengths:
		...
		segment = df.xs(scaffold_name, level='reference_name'). \
			sort_values('mean_weighted_center') \
...
def plot(df, reference, agp, filename):

	for scaffold_name in agp:
		for query_name in agp[scaffold_name]:
			if query_name in df.index.get_level_values('query_name').unique():
				df.loc[query_name, 'scaffold_name'] = scaffold_name

	df['scaffold_weighted_center'] = df.groupby(
		by = ['reference_name', 'scaffold_name'])['mean_weighted_center'].transform(np.mean)
	...
	with PdfPages(filename) as pdf:
		...
		for reference_name in reference:
			...
			light_colors = ['#64b0e4', '#ff9e49', '#6bd66b', '#e36869', '#a782c9']
			...
			if reference_name in df.index.get_level_values('reference_name').unique():
				...
				# For each scaffold. 
				for scaffold_name, scaffold in section.groupby('scaffold_name', sort = False):
					j = 0
					...
					for query_name in agp[scaffold_name]:
						...
						if query_name in section.index.get_level_values('query_name').unique():
							...
							# Add scaffold label.
							if j == 0:
								x = reference_end + proceeding_length + (reference_maximum * 0.01)
								text = plt.text(
									x = x,
									y = i + 0.375,
									s = scaffold_name, 
								...
								text_end = text.get_window_extent().transformed(\
									plt.gca().transData.inverted()).x1
...
def main(reference, assembly, scaffold_file, outdir, threads, min_length, min_coverage):

	alignment = run_mummer(
		reference = reference,
		assembly = assembly,
		threads = threads)
	...
	'''
	For each query sequence, group all of its alignments to a particular 
	reference by summing them together. Then find the maximum query alignment
	length for any given reference for each query. This is the assignment for 
	that particular contig. Then to find the orientation, we take that
	'''
	...
	filtered['signed_query_alignment_length'] = \
		filtered['query_alignment_length'] * \
		filtered['query_orientation']
	filtered['orientation'] = np.where(filtered.groupby(
		by = ['query_name','reference_name']) \
		['signed_query_alignment_length'].transform(np.mean) > 0, '+', '-')
	...
	# alignment. 
	filtered['weighted_center'] = filtered[['reference_start', 'reference_end']].mean(axis = 1) * \
	...
	filtered['mean_weighted_center'] = filtered.groupby(
		by = ['query_name','reference_name'])['weighted_center'].transform(sum)
