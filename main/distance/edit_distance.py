...
def check_files(reference, assembly):
...
# Check if AGP files exist. 
def check_agp(assembly):
...
def main():
    ...
    if check_agp(arguments.assembly):
        reference_agp, assembly_agp = check_agp(
            assembly = arguments.assembly)
        preprocess = False
    ...
    distances = scaffold_distance.main(
        reference = reference_agp, 
        assembly = assembly_agp)
